<script lang="ts">
	...
	import {
		...
		filterDisallowedTags, sanitizeUrl, sanitizeHtml, type AnyNode,
	} from '@reach/markdown';
	import { t } from '$lib/state/i18n.svelte';
	import { cjkWbrHtml, cjkWbrSegments } from '$lib/utils/markdown';
...
{:else if node.name === "Text"}
	<!-- segments carry the CJK closing punctuation at their end; <wbr> between
	     them restores the break opportunities UAX #14 LB13 suppresses -->
	{#each cjkWbrSegments(textContent(node)) as segment, i (i)}{#if i > 0}<wbr />{/if}{segment}{/each}
...
{:else if node.name === "URL"}
	...
	<a href={sanitizeUrl(bareAutolinkHref(node.content ?? ""))} target="_blank" rel="noreferrer">{node.content}</a>
...
{:else if node.name === "InlineCode"}
	<code class="md-inline">{codeSpanText(node, inTable)}</code>
{:else if node.name === "CodeBlock" || node.name === "FencedCode"}
	{@const language = codeBlockLanguage(node)}
	<!-- codeBlockText keeps each line's terminator; drop the trailing newline
	     so the copied text matches the fenced source exactly -->
	{@const text = codeBlockText(node).replace(/\n$/, "")}
	<div class="md-code">
		<div class="md-code-head">
			<span>{language || "code"}</span>
			<button type="button" class="md-copy" data-code={encodeURIComponent(text)}>{t('agent.copy')}</button>
		</div>
...
{:else if node.name === "Autolink"}
	...
	<a href={sanitizeUrl(autolinkHref(url))} target="_blank" rel="noreferrer">{url}</a>
{:else if node.name === "HTMLBlock" || node.name === "CommentBlock" || node.name === "ProcessingInstructionBlock"}
	...
	{@html cjkWbrHtml(sanitizeHtml(renderRawBlock(node)))}
{:else if node.name === "HTMLTag" || node.name === "Comment" || node.name === "ProcessingInstruction"}
	...
	{@html cjkWbrHtml(sanitizeHtml(filterDisallowedTags(node.content ?? "")))}
{:else if node.name === "Link"}
	{#if hasDest(node)}
		{@const attrs = linkAttrs(node)}
		<a href={sanitizeUrl(attrs.url ?? "")} title={attrs.title || null} target="_blank" rel="noreferrer">{#each textChildren(node) as child (child.id)}<MarkdownNode node={child} />{/each}</a>
	{:else}
...
{:else if node.name === "Image"}
	{#if hasDest(node)}
		{@const attrs = linkAttrs(node)}
		<img src={sanitizeUrl(attrs.url ?? "", true)} alt={imageAlt(node)} title={attrs.title || null} />
	{:else}
		{#each node.children as child (child.id)}
			{#if child.name === "LinkLabel"}
				{unescapeString(child.content ?? "")}
