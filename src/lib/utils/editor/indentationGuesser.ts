 ...
 *---------------------------------------------------------------------------------------------
 *  Note: This function was extracted from VS Code and modified for this project.
 *--------------------------------------------------------------------------------------------*/
...
export function guessIndentation(source: string, defaultTabSize: number, defaultInsertSpaces: boolean): IGuessedIndentation {
	const lines = source.split('\n');
	// Look at most at the first 10k lines
	const linesCount = Math.min(lines.length, 10000);
	...
	for (let lineNumber = 1; lineNumber <= linesCount; lineNumber++) {
		const currentLineLength = lines[lineNumber - 1].length;
		...
		for (let j = 0, lenJ = currentLineLength; j < lenJ; j++) {
			const charCode = (useCurrentLineText ? currentLineText.charCodeAt(j) : lines[lineNumber - 1].charCodeAt(j));
