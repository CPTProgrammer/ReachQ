...
/** JSON drips in 8–16 char chunks; oversized payloads scale the chunk size
 *  so the stream still finishes in ~maxChunks steps instead of a minute. */
function jsonChunks(json: string, maxChunks = 120): string[] {
	const base = Math.max(8, Math.ceil(json.length / maxChunks));
	const out: string[] = [];
	let i = 0;
	while (i < json.length) {
...
/** Path of the generated module the long-diff scenario writes. */
const LONG_WRITE_PATH = 'src/lib/generated/policy-matrix.ts';

const LONG_WRITE_ACTIONS = ['read', 'write', 'execute', 'delete'] as const;

/** A ~330-line TypeScript module plus its all-adds structured diff, so the
 *  approval card towers past the viewport (sticky approval bar, unbounded
 *  diff height). Generated in a loop — nobody wants 300 literal lines here. */
function buildLongWrite(): { content: string; preview: DiffPreview } {
	const lines: string[] = [
		'//! GENERATED by `reach policy build` — do not edit by hand.',
		'',
	...
	for (let i = 1; i <= 40; i++) {
		...
		lines.push(
			`export const rule${num}: PolicyRule = {`,
			`\tid: 'rule-${num}',`,
			`\teffect: '${i % 7 === 0 ? 'deny' : 'allow'}',`,
			`\tresource: 'srv:/srv/data/shard-${String(i % 12).padStart(2, '0')}',`,
			`\tactions: [${LONG_WRITE_ACTIONS.slice(0, (i % 4) + 1)
				.map((a) => `'${a}'`)
				.join(', ')}]`,
			'};',
			''
	...
	lines.push('export const ALL_RULES: readonly PolicyRule[] = [');
	for (let i = 1; i <= 40; i++) {
		lines.push(`\trule${String(i).padStart(2, '0')},`);
	}
	lines.push('];', '');
	const content = lines.join('\n');
	const preview: DiffPreview = {
		path: LONG_WRITE_PATH,
		isNewFile: true,
		hunks: [
			{
				oldStart: 0,
				newStart: 1,
				lines: lines.map((text, i): DiffPreviewLine => ({ kind: 'add', text, newNo: i + 1 }))
			}
		]
	};
...
class MockAgentBackend implements AgentBackend {
	...
	private makeCtx(
		...
		private scenarios: Record<string, MockScenario & { prompt?: string }> = {
			...
			'tool-write-long-diff': {
				label: 'write_file long diff',
				description:
					'A new ~330-line module streams in, then waits for approval — the unbounded diff card towers past the viewport with sticky actions.',
				prompt: 'Generate the permission policy matrix module.',
				run: async (ctx) => {
					const { content, preview } = buildLongWrite();
					const msg = ctx.beginAssistant();
					await ctx.sleep(300);
					const call = await ctx.streamToolCall(msg, 'write_file', {
						path: LONG_WRITE_PATH,
						content
					});
					const ok = await ctx.requestApproval(call, LONG_WRITE_PATH, [], {
						diff: preview,
						path: LONG_WRITE_PATH
					});
					if (!ok) {
						...
						await ctx.sleep(600);
						ctx.setToolStatus(call, 'success', {
							llmText: `Created ${LONG_WRITE_PATH} (${preview.hunks[0].lines.length} lines).`,
							isError: false,
							uiPayload: { diff: preview, path: LONG_WRITE_PATH }
						});
					}
					ctx.addUsage(420);
					ctx.finishMessage(msg, 1);
					...
					await ctx.streamText(
						follow,
						ok
							? 'Done — import `ALL_RULES` from the generated module to enforce the policy matrix.'
